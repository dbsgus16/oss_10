...
##### 재고관리
재고관리 EPNeRxt모듈에서 농장내 품목과 재배 주기에 대한 전체적인 개요를 확인할수있습니다. 판매, 구매, 재고 관리와 같은 모듈을 제공하며, 어느 기업에나 필요한 전자세금계산서 발행, 자동 결제 정산 기능 등을 포함합니다. 이러한 기능들은 견적, 주문, 배송, 청구서 발행, 결제 등의 작업을 순차적으로 처리할 수 있게 해줍니다. 또한, Excel 배치 업로드를 통한 재고 조정 등록이 가능하며, 사용자는 Excel 데이터 등록 매뉴얼을 참조해야 합니다.
